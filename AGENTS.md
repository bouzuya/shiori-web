...
マークアップ / レイアウトを変更・提案するときは、その回の論点だけを局所最適せず、既存の確定事項全体への退行を点検し、満たす点・退行する点・未決のトレードオフを明示してから出すこと。点検する観点:

- 明示済みの要件と、これまでの決定 / 撤回履歴との整合 (黙って覆さない。覆すなら代償を述べる)
- スクロール / 固定のレイアウト要件とスクロールモデル (何がスクロールし何が固定か。変更の意図と副作用)
- ARIA ランドマーク・HTML 要素のセマンティクス (要素の役割と中身の一致、コントロールと操作対象の関連)。レイアウト都合だけで要素の意味的な所属を決めない
- CSS の locality とシェル構造の全ページ統一 (別軸として両立させる)
- アクセシビリティの細部 (`<nav>` の `aria-label` 等)

## コミット運用

- コミットは人間が行う。エージェントは `git commit` を実行しない
- エージェントは `git config` の変更も行わない
- エージェントは各ステップを format / lint / test が通る状態で止め、レビューとコミットは人間に委ねる
