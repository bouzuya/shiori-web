...
mod tests {
    ...
    async fn with_base_path_next_page_link_has_no_trailing_slash() -> ::anyhow::Result<()> {
        ...
        let state = AppState::new(
            ...
            TEST_COOKIE_SIGNING_SECRET,
            crate::test_helpers::mock_id_token_verifier(),
            ::std::sync::Arc::new(MockAuthorizationCodeClient::new(unique_user_id())),
            firestore_user_repo()?,
            firestore_user_settings_reader()?,
            firestore_user_settings_repository()?,
        );
    ...
    async fn with_base_path_root_contains_base_path_links() -> ::anyhow::Result<()> {
        ...
        let state = AppState::new(
            ...
            TEST_COOKIE_SIGNING_SECRET,
            crate::test_helpers::mock_id_token_verifier(),
            ::std::sync::Arc::new(MockAuthorizationCodeClient::new("base_path_links_user")),
            firestore_user_repo()?,
            firestore_user_settings_reader()?,
            firestore_user_settings_repository()?,
        );
    ...
    async fn with_base_path_root_link_has_no_trailing_slash() -> ::anyhow::Result<()> {
        ...
        let state = AppState::new(
            ...
            TEST_COOKIE_SIGNING_SECRET,
            crate::test_helpers::mock_id_token_verifier(),
            ::std::sync::Arc::new(MockAuthorizationCodeClient::new(
                "base_path_trailing_slash_user",
            )),
            firestore_user_repo()?,
            firestore_user_settings_reader()?,
            firestore_user_settings_repository()?,
